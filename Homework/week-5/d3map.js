...
main d3.js file changing the color of countries, based on CO2 emmision.
*******/
...
// fill object with countries and data
for (var i = 0; i < countryCodes.length; i++) {
	for (var j = 0; j < emissionData.length; j++) {
		if (emissionData[j][0] === countryCodes[i][2]) {
			data[countryCodes[i][1]] = { fillKey: determineColor(emissionData[j][1]),
										 emission: emissionData[j][1]}
		}
	}
}

// gives fillKey values according to emission value
function determineColor(emissionValue) {
	if (emissionValue === -1.0) {
		return "noData"
	} else if (emissionValue >= 0 && emissionValue <= 2.5) {
		return "low"
	} else if (emissionValue > 2.5 && emissionValue <= 7.5) {
		return "medium"
	} else {
		return "high"
	}
...
document.addEventListener('click', function(e) {
	var clickedID = e.target.__data__.id;
	var clickedValue = data[clickedID].emission;
	...
	for (var key in data) {
		var difference = clickedValue - data[key].emission;
